...
const config = require("../config"); // ✅ FIXED (no more config.json)
...
class CommandHandler {
  constructor(member, guild, command, cooldowns, interaction, args) {
    ...
    this.args = args;
    this.whole_command = this.args.sub_command
      ? `${this.command.name} ${this.args.sub_command}`
      : this.command.name;
  ...
  async handle() {
    if (!this.command.dontUseDB) {
      this.dbUser =
        (await userModel.findOne({ id: this.member.id })) ??
        createUserDefaults({ id: this.member.id });
    ...
    if (this.command.cooldown) {
      cooldownResult = this.command.dontUseDB
        ? this.handleCooldown()
        : this.handleCooldown(this.dbUser);
  ...
  isUserRestricted(user) {
    ...
    if (
      ...
      const reason = user.blacklisted
        ? "You're blacklisted."
        : this.command.staffOnly && !user.staff
        ? "You don't have access to use this command."
        : this.command.requireLink && !user.didLink
        ? "This requires you to link."
        : this.command.premium && !user.premium
        ? "This command requires premium."
        : this.command.notUpdated
        ? "This command is not updated to the current version."
        : "";
  ...
  handleCooldown(user) {
    ...
    if (typeof this.command.cooldown === "object") {
      cooldownDuration =
        this.command.cooldown[sub_command] ??
        this.command.cooldown.global ??
        0;
    } else {
    ...
    if (
      Object.keys(cooldownMap).length !== 0 &&
      cooldownMap[this.whole_command]
    ) {
      rateLimitEnd = ms(cooldownMap[this.whole_command] - now, {
        long: true
      });

      if (
        !rateLimitEnd.includes("ms") &&
        cooldownMap[this.whole_command] > now
      ) {
