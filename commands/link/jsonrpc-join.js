...
const { cleanLeftovers } = require("../../common/Util.js");
...
module.exports = {
  name: "jsonrpc-join",
  description: 'Join a NETHERNET_JSONRPC realm to leave afterwards (Success rate: 60-80%; may not always work!)',
  ...
  execute: async (interaction, args, dbUser, embed, clearCooldown) => {
    ...
    switch (realmIP.networkProtocol) {
      ...
      case "NETHERNET":
      case "DEFAULT":
        embed.description = `Operation failed\nInput: **${input}**\nYou can only use this command on realms with the NETHERNET_JSONRPC protocol. This realm is using ${realmIP.networkProtocol}.`;
        embed.fields[0].value = ":red_circle:";
    ...
    let clients = await new bpClient(address, dbUser, realm, configuration).connect();
    ...
    for (const c of clients) {
      ...
      if (typeof c === "string") {
        switch (c) {
          ...
          case "Bad IP":
          case "Bad Port":
            clientValue = `Invaild IP/Port has been provided. (${c})`;
            break;
          case "No Nethernet Network ID":
          case "Bad Nethernet Network ID":
            clientValue = `Invaild Nethernet Network ID has been provided. (${c})`;
            break;
          ...
          case "Currently Doing Operation":
            ...
            break;
          default:
            clientValue = `Failed to connect to the realm for an unknown reason. Please try again later or ask the developer(s) for more information. (${c})`;
            break;
        }
    ...
    timeout = setTimeout(async () => {
      ...
      embed.description = `Successfully connected to **${realm.name}**\nInput: **${input} ${/^\d+$/.test(input) ? "" : `(${realm.id})`}**\nProtocol: **${realmIP.networkProtocol}**`;
