  ...
  execute: async (interaction, args, dbUser, embed, clearCooldown) => {
    ...
    client.once("start_game", async (packet) => {
      embed.fields.push({
        id: 3,
        name: "Seed",
        value: `Converted Seed: \`${convertSeed(packet.seed)}\`\nServer Seed: \`${packet.seed}\``,
        inline: true
      })
            ...
            components: [
              {
                ...
                style: 5,
                label: "Chunkbase",
                url: `https://www.chunkbase.com/apps/seed-map#seed=${convertSeed(packet.seed)}&platform=bedrock_26_0&dimension=overworld&x=0&z=0&zoom=0`
              }
            ]
          }
        ]
