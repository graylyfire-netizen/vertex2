...
module.exports = {
  name: "friends",
  description: "Invite some of LESTER's friends",
  ...
  options: [
    ...
    },
    {
      type: 5,
      name: "ecblock",
      description: "Enable ec-block while doing this operation."
    },
  ],
  execute: async (interaction, args, dbUser, embed, clearCooldown) => {
    let { input, time, ad, ecblock } = args;
    const { user } = interaction;

    let address = {};

    if (typeof ad === "undefined") ad = true;
    if (typeof ecblock === "undefined") ecblock = false;
    ...
    client.on("play_status", async (packet) => {
      ...
      let CSCConfig = {
        ssbp: { enabled: ecblock, type: 3 },
        batchOptions: { enabled: true, count: 500 },
        name: { enabled: true, value: "§1§lLESTER © | .gg/RUCVhyfn5z" },
        listeners: { enabled: false },
        massjoin: { enabled: true }
      }

      for (let i = 0; i < 3; i++) client.createSubClient(i, CSCConfig);
