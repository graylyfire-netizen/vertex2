...
module.exports = {
  name: "containers",
  description: "Retrieve the positions of all interacted containers",
  ...
  execute: async (interaction, args, dbUser, embed, clearCooldown) => {
    ...
    client.on("block_event", (packet) => {
      if (logs.includes(JSON.stringify(packet.position))) return;

      logs.push(JSON.stringify(packet.position))
    })
