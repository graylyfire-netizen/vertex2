...
class bpClient {
    constructor(address, dbUser, realm, configuration) {
        this.address = address;
        this.dbUser = dbUser;
        this.realm = realm;
        this.configuration = configuration ?? {};
        this.clients = [];
        this.api = new RealmAPI(dbUser.id);
        this.intervals = [];
    ...
    validate() {
        ...
        if (this.clients.length === 0) {
            ...
            if (ConcurrentRealms.includes(this.realm.id)) return "Concurrent Operation";

            ConcurrentUsers.set(this.dbUser.id, 1);
            ConcurrentRealms.push(this.realm.id);
        }
    ...
    #setupPresence() {
        ...
        const updatePresence = () => {
            this.api.sendPresence({
                ...
                activity: {
                    richPresence: {
                        id: `Realm_${this.realm.gamemode}`,
                        scid: this.userFlow.scid
                    }
                }
            });

            this.api.sendInGamePresence(this.realm, true);
        }
    ...
    disconnect(client) {
        ...
        if (this.clients.length === 0) {
            const rIndex = ConcurrentRealms.indexOf(this.realm.id);
            ...
            this.api.sendInGamePresence(this.realm, false);
